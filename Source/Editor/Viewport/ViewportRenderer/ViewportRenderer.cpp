...
ViewportRenderer::ViewportRenderer(Appstate& appstate, Viewport* viewport, WorldRenderer& worldRenderer) 
    ...
    GEventBUS.Subscribe(SDL_EVENT_WINDOW_RESIZED, [this]() {
        ...
        // World Renderer resize handling
        SDL_ReleaseGPUTexture(this->appstate.device, this->worldRenderer.depthStencilTexture);
        this->worldRenderer.depthStencilTexture = nullptr;

        SDL_ReleaseGPUTexture(this->appstate.device, this->worldRenderer.msaaTexture);
        this->worldRenderer.msaaTexture = nullptr;

        SDL_ReleaseGPUTexture(this->appstate.device, this->worldRenderer.offscreenTexture);
        this->worldRenderer.offscreenTexture = nullptr;

        this->worldRenderer.CreateDepthStencil();
        this->worldRenderer.CreateMSAATexture();
        this->worldRenderer.CreateOffscreenTexture();

        // Viewport Renderer resize handling
